...
const note = ref('Podaj poprawne dane')
  ...
  axios
    .get('/login', {
      params: {
        ...
        first: loginData.pass.charAt(0),
        last: loginData.pass.charAt(loginData.pass.length - 1)
      }
    })
    .then((res) => {
...
function getToken() {
  let passMD5 = MD5(loginData.pass)
  axios
    .get('/getToken', {
      params: {
        nickName: loginData.nickName,
    ...
    .then((res) => {
      if (res.data.status == '200') {
        session.set(res.data.token, res.data.userId)
        router.push('/dashboard')
      } else {
        console.log('Nie otrzymano danych tokena!')
      }
    })
}

function wait(time) {
  note.value = `Poczekaj ${time} sekund(y).`
  if (time > 0) {
    setTimeout(() => {
      wait(time - 1)
    }, 1000)
  } else {
    note.value = 'Możesz spróbować pownownie'
  }
}
</script>
